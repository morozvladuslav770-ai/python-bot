...
async def date_chosen(callback: types.CallbackQuery, state: FSMContext):
    ...
    slots_kb = get_time_slots_kb(chosen_date)
    if len(slots_kb.inline_keyboard) == 0:
        await callback.message.edit_text(
            f"😔 На {chosen_date} вже немає вільних слотів.\nОберіть, будь ласка, іншу дату:",
...
async def phone_entered(message: types.Message, state: FSMContext):
    ...
    # 0. ОСТАННЯ ПЕРЕВІРКА: чи слот не зайняли, поки клієнт вводив ім'я/телефон
    if database.is_slot_taken(b_date, b_time):
        await message.answer(
            "😔 На жаль, поки ви вводили дані, цей час щойно зайняли.\n"
            "Будь ласка, оберіть запис заново:",
